...
$commonModuli = array_reduce($monkeyConfiguration, fn(int $total, array $configuration) => $total * $configuration['test'], 1);
for ($i = 0; $i < 10000; ++$i) {
    for ($m = 0; $m < count($monkeyConfiguration); ++$m) {
        foreach ($monkeyConfiguration[$m]['items'] as $idx => $item) {
            $worryLevel = (int) eval('return ' . str_replace('old', $item, $monkeyConfiguration[$m]['operation']) . ';') % $commonModuli;
